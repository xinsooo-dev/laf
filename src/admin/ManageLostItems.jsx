...
import { Search, Eye, MapPin, Calendar, User, Package, Maximize2, ZoomIn, ZoomOut, X, Edit } from 'lucide-react';
import { API_ENDPOINTS, getAssetUrl } from '../utils/api';

function ManageLostItems() {
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center py-12">
                ...
                <span className="ml-2 text-gray-600">Loading lost items...</span>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4 sm:space-y-6 w-full">
            <div className="hidden md:flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800 break-words">Manage Lost Items</h2>
                <div className="flex items-center space-x-2 text-xs sm:text-sm text-gray-600 flex-shrink-0">
                    <Package className="h-4 w-4 flex-shrink-0" />
                    <span className="whitespace-nowrap">{filteredItems.length} lost items</span>
                </div>
            </div>
            ...
            {filteredItems.length === 0 ? (
                <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-gray-800 mb-2">No lost items found</h3>
                    <p className="text-gray-600">No lost items match your current filters.</p>
                </div>
            ) : (
            ...
            {showModal && selectedItem && (
                <>
                    ...
                    >
                        ...
                        >
                            {/* Modal Header - Fixed */}
                            <div className="bg-gradient-to-r from-red-600 to-red-700 text-white p-2.5 md:p-4 flex-shrink-0">
            ...
            {isFullscreen && selectedItem?.image_path && (
                <div
                    className="fixed inset-0 bg-black bg-opacity-95 z-[100] flex items-center justify-center p-2 md:p-4"
                    onClick={() => setIsFullscreen(false)}
                >
                    <button
                        onClick={() => setIsFullscreen(false)}
                        className="absolute top-2 right-2 md:top-4 md:right-4 text-white hover:text-gray-300 bg-black bg-opacity-50 hover:bg-opacity-70 p-2 md:p-3 rounded-lg transition-all z-10"
                    ...
                    {/* Zoom Controls */}
                    <div className="absolute bottom-4 md:bottom-8 left-1/2 transform -translate-x-1/2 flex gap-2 md:gap-3 z-[101]">
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
                                setZoomLevel(prev => Math.max(0.5, prev - 0.25));
                            }}
                            className="bg-gray-800 hover:bg-gray-700 text-white p-2 md:p-3 rounded-lg transition-all flex items-center gap-1 md:gap-2 shadow-lg border border-gray-600 disabled:bg-gray-600 disabled:cursor-not-allowed"
                            title="Zoom Out"
                        ...
                        <div className="bg-gray-800 text-white px-2 md:px-4 py-2 md:py-3 rounded-lg flex items-center shadow-lg border border-gray-600">
                            <span className="text-xs md:text-sm font-medium">{Math.round(zoomLevel * 100)}%</span>
                        </div>
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
